...
router = APIRouter(prefix="/api/v1/dashboard", tags=["dashboard-admin"])
...
async def list_nodes(authorization: str | None = Header(default=None), db: AsyncSession = Depends(get_db)):
    auth(authorization)
    return (await db.execute(select(Node).order_by(Node.name))).scalars().all()

@router.post("/nodes")
async def create_node(payload: NodeIn, authorization: str | None = Header(default=None), db: AsyncSession = Depends(get_db)):
    auth(authorization)
    if await db.scalar(select(Node).where(Node.name == payload.name)):
        raise HTTPException(409, "Node name already exists")
    node = Node(name=payload.name, kind=payload.kind, endpoint=payload.endpoint,
                enabled=payload.enabled, maintenance=payload.maintenance,
                capacity=payload.capacity, metadata_json=payload.metadata)
    db.add(node)
    db.add(AuditLog(action="dashboard.node.create", target=payload.name, data={"kind": payload.kind}))
...
async def update_node(node_id: UUID, payload: NodeIn, authorization: str | None = Header(default=None), db: AsyncSession = Depends(get_db)):
    ...
    if await db.scalar(select(Node).where(Node.name == payload.name, Node.id != node_id)):
        raise HTTPException(409, "Node name already exists")
    node.name = payload.name; node.kind = payload.kind; node.endpoint = payload.endpoint
    node.enabled = payload.enabled; node.maintenance = payload.maintenance
    node.capacity = payload.capacity; node.metadata_json = payload.metadata
    db.add(AuditLog(action="dashboard.node.update", target=str(node_id), data={"name": payload.name}))
    await db.commit(); await db.refresh(node)
    return node
...
async def set_maintenance(node_id: UUID, payload: MaintenanceIn, authorization: str | None = Header(default=None), db: AsyncSession = Depends(get_db)):
    ...
    if not node: raise HTTPException(404, "Node not found")
    node.maintenance = payload.maintenance
    db.add(AuditLog(action="dashboard.node.maintenance", target=str(node_id), data={"maintenance": payload.maintenance}))
    await db.commit(); return {"ok": True, "maintenance": node.maintenance}
...
async def set_enabled(node_id: UUID, payload: BoolIn, authorization: str | None = Header(default=None), db: AsyncSession = Depends(get_db)):
    ...
    db.add(AuditLog(action="dashboard.node.enabled", target=str(node_id), data={"enabled": payload.enabled}))
    await db.commit(); return {"ok": True, "enabled": node.enabled}
...
async def retry_server(server_id: UUID, authorization: str | None = Header(default=None), db: AsyncSession = Depends(get_db)):
    ...
    job.status = "queued"; job.error = None; server.status = "provisioning"
    db.add(AuditLog(action="dashboard.server.retry", target=str(server_id)))
    await db.commit(); return {"ok": True}

@router.get("/users")
async def list_users(authorization: str | None = Header(default=None), limit: int = 100, db: AsyncSession = Depends(get_db)):
    auth(authorization); limit = min(max(limit, 1), 500)
...
async def deployments(authorization: str | None = Header(default=None), limit: int = 100, db: AsyncSession = Depends(get_db)):
    auth(authorization); limit = min(max(limit, 1), 500)
    return (await db.execute(select(DeploymentJob).order_by(DeploymentJob.created_at.desc()).limit(limit))).scalars().all()
...
async def security_summary(authorization: str | None = Header(default=None)):
    ...
    return {
        ...
        "trusted_hosts_configured": bool(settings.trusted_host_list),
        "cors_origins": settings.cors_origin_list,
        "rate_limit_per_minute": settings.rate_limit_per_minute,
        "max_deployment_attempts": settings.max_deployment_attempts,
        "credential_encryption_configured": bool(settings.credential_encryption_key),
        "internal_secret_configured": bool(settings.internal_api_secret),
    }
